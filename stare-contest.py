...
# Mouse callback function for button 2
def button2_callback(event, x, y, flags, param):
    if event == cv2.EVENT_LBUTTONDOWN:
        print("Button 2 clicked")

def button_panel(args):
    window_name = 'Buttons'
    cv2.namedWindow(window_name)

    cv2.createButton('B1', button1_callback, None, cv2.QT_PUSH_BUTTON, 1)
    cv2.createButton('B2', button2_callback, None, cv2.QT_PUSH_BUTTON, 1)

    while True:
        cv2.imshow(window_name, 255 * np.ones((300, 500, 3),
                                              dtype=np.uint8))  # White
...
if __name__ == '__main__':
    ...
    args = []
    for i,id_ in enumerate(device_ids):
    ...
    try:
        with ProcessPoolExecutor() as executor:
            ...
            buttons_future = executor.submit(button_panel, [])
    except KeyboardInterrupt:
